<template>
  <div>
    <template v-if="loggedIn && current_template==='template_index'">
      <h1 >Time manager</h1>
      ...
      <div class="array-workingtime" v-for="user in users" v-bind:key="user.id">
        <ClockManager :user="user" />
      </div>
      <h2 class="h-title">Data Vizualisation</h2>
      <template v-if="token != ''">
      <ChartManager :token="token"/>
      </template>
    </template>
    <template v-if="!loggedIn || current_template==='template_login'">
      <Login />
    </template>
  </div>
</template>
